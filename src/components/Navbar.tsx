      ...
      <>
        <nav className="fixed w-full bg-white/95 backdrop-blur-md z-50 shadow-lg border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between h-16 items-center">
              ...
              <div className="hidden md:flex items-center space-x-14">
                ...
                <div className="relative dropdown-container">
                  <button
                      onClick={() => setDesktopDropdown(desktopDropdown === 'industries' ? null : 'industries')}
                      className="flex items-center relative text-gray-700 hover:text-blue-600 transition-all duration-300 ease-in-out group"
                  >
                    Industries 
                    <ChevronDown size={16} className={`ml-1 transition-transform duration-300 ${desktopDropdown === 'industries' ? 'rotate-180' : ''}`} />
                    <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-blue-600 transition-all duration-300 group-hover:w-full"></span>
                  </button>
                ...
                <div className="relative dropdown-container">
                  <button
                      onClick={() => setDesktopDropdown(desktopDropdown === 'products' ? null : 'products')}
                      className="flex items-center relative text-gray-700 hover:text-blue-600 transition-all duration-300 ease-in-out group"
                  >
                    Products 
                    <ChevronDown size={16} className={`ml-1 transition-transform duration-300 ${desktopDropdown === 'products' ? 'rotate-180' : ''}`} />
                    <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-blue-600 transition-all duration-300 group-hover:w-full"></span>
                  </button>
                ...
                <Link to="/contact">
                  <Button variant="default" className="transition-transform hover:scale-105 bg-blue-600 hover:bg-blue-700">
                    Contact Us
                  </Button>
                </Link>
              </div>
            ...
            <motion.div
                className={`md:hidden ${isOpen ? "block" : "hidden"} bg-white/95 backdrop-blur-md border-t border-gray-200`}
              ...
              >
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                  <div className="flex justify-between space-x-8">
                    <div>
                      <span className="block font-semibold text-gray-800 mb-2">Main Links</span>
                      <button onClick={() => handleNavigation("/")} className="block py-1 text-gray-600 hover:text-blue-600 transition-colors duration-300">Home</button>
                      <button onClick={() => handleScrollToSection("about")} className="block py-1 text-gray-600 hover:text-blue-600 transition-colors duration-300">About</button>
                      <button onClick={() => handleNavigation("/contact")} className="block py-1 text-gray-600 hover:text-blue-600 transition-colors duration-300">Contact Us</button>
                    </div>
                    <div>
                      <span className="block font-semibold text-gray-800 mb-2">Industries</span>
                      {industries.map((industry) => (
                          <button key={industry.id} onClick={() => handleNavigation(`/industries/${industry.id}`)} className="block py-1 text-gray-600 hover:text-blue-600 transition-colors duration-300">
                            {industry.name}
                          </button>
                      ))}
                    </div>
                    <div>
                      <span className="block font-semibold text-gray-800 mb-2">Products</span>
                      {products.map((product) => (
                          <button key={product.id} onClick={() => handleNavigation(`/products/${product.id}`)} className="block py-1 text-gray-600 hover:text-blue-600 transition-colors duration-300">
                            {product.name}
                          </button>
                      ))}
                    </div>
